...
- `device_id` (String)
- `name` (String) Configured hostname assigned to the access point
- `site_id` (String) Site where this access point is assigned

### Optional

- `aeroscout` (Attributes) Location integration settings for AeroScout on this access point (see [below for nested schema](#nestedatt--aeroscout))
- `airista` (Attributes) Location integration settings for Airista on this access point (see [below for nested schema](#nestedatt--airista))
- `ble_config` (Attributes) Bluetooth Low Energy beacon and asset settings for this access point (see [below for nested schema](#nestedatt--ble_config))
- `centrak` (Attributes) Location integration settings for Centrak on this access point (see [below for nested schema](#nestedatt--centrak))
- `client_bridge` (Attributes) Wireless client bridge settings for this access point (see [below for nested schema](#nestedatt--client_bridge))
...
- `enable_unii_4` (Boolean) Whether U-NII-4 channels (169, 173, 177) are enabled on this access point
- `esl_config` (Attributes) Electronic shelf label integration settings for this access point (see [below for nested schema](#nestedatt--esl_config))
- `flow_control` (Boolean) For some AP models, flow_control can be enabled to address some switch compatibility issue
- `height` (Number) Installation height of the AP, in meters
- `ip_config` (Attributes) Management IP addressing settings for this access point (see [below for nested schema](#nestedatt--ip_config))
- `lacp_config` (Attributes) Link aggregation settings for supported AP Ethernet uplinks (see [below for nested schema](#nestedatt--lacp_config))
- `led` (Attributes) Indicator light behavior settings for this access point (see [below for nested schema](#nestedatt--led))
- `locked` (Boolean) Whether this map is considered locked down
- `map_id` (String) Map where the device belongs to
- `mesh` (Attributes) Wireless mesh role and band settings for this access point (see [below for nested schema](#nestedatt--mesh))
- `mqtt_config` (Attributes) MQTT broker publishing settings for this access point (see [below for nested schema](#nestedatt--mqtt_config))
- `notes` (String) Any notes about this AP
- `ntp_servers` (List of String) NTP servers used by this access point
- `orientation` (Number) AP orientation in degrees from 0 to 359, where 0 is up and 90 is right
...
- `pwr_config` (Attributes) Power negotiation and peripheral power settings for this access point (see [below for nested schema](#nestedatt--pwr_config))
- `radio_config` (Attributes) Radio configuration overrides for this access point (see [below for nested schema](#nestedatt--radio_config))
- `uplink_port_config` (Attributes) Authentication and failover behavior for AP uplink ports (see [below for nested schema](#nestedatt--uplink_port_config))
- `usb_config` (Attributes) Legacy USB integration settings for this access point (see [below for nested schema](#nestedatt--usb_config))
- `uwb_config` (Attributes) UWB RTLS / OMLOX asset-visibility settings; overrides the device profile and site-level `uwb_config` (see [below for nested schema](#nestedatt--uwb_config))
- `vars` (Map of String) Variable values that override site variables for this access point
- `x` (Number) Horizontal map position of the AP, in pixels
- `y` (Number) Vertical map position of the AP, in pixels
- `zigbee_config` (Attributes) Zigbee radio and network settings for this access point (see [below for nested schema](#nestedatt--zigbee_config))

### Read-Only

- `image1_url` (String) First custom image URL associated with the access point
- `image2_url` (String) Second custom image URL associated with the access point
- `image3_url` (String) Third custom image URL associated with the access point
- `mac` (String) Access point MAC address used to identify the device
...
- `enabled` (Boolean) Whether to enable Centrak config
...
- `auth` (Attributes) Credentials and security mode used when the AP connects as a wireless client bridge (see [below for nested schema](#nestedatt--client_bridge--auth))
- `enabled` (Boolean) When acted as client bridge:
  * only 5G radio can be used
  * will not serve as AP on any radios
- `ssid` (String) Uplink SSID used by the AP when client bridge mode is enabled
