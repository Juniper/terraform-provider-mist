...
func OrgGatewaytemplatesDataSourceSchema(ctx context.Context) schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"org_gatewaytemplates": schema.SetNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					...
					CustomType: OrgGatewaytemplatesType{
						ObjectType: types.ObjectType{
							AttrTypes: OrgGatewaytemplatesValue{}.AttributeTypes(ctx),
						},
					},
				},
				Computed: true,
...
var _ basetypes.ObjectTypable = OrgGatewaytemplatesType{}
...
func (t OrgGatewaytemplatesType) Equal(o attr.Type) bool {
	other, ok := o.(OrgGatewaytemplatesType)
...
func (t OrgGatewaytemplatesType) String() string {
	return "OrgGatewaytemplatesType"
}
...
func (t OrgGatewaytemplatesType) ValueFromObject(ctx context.Context, in basetypes.ObjectValue) (basetypes.ObjectValuable, diag.Diagnostics) {
	...
	return OrgGatewaytemplatesValue{
...
func NewOrgGatewaytemplatesValue(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) (OrgGatewaytemplatesValue, diag.Diagnostics) {
	...
	for name, attributeType := range attributeTypes {
		...
		if !ok {
			diags.AddError(
				"Missing OrgGatewaytemplatesValue Attribute Value",
				"While creating a OrgGatewaytemplatesValue value, a missing attribute value was detected. "+
					"A OrgGatewaytemplatesValue must contain values for all attributes, even if null or unknown. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("OrgGatewaytemplatesValue Attribute Name (%s) Expected Type: %s", name, attributeType.String()),
			)
		...
		if !attributeType.Equal(attribute.Type(ctx)) {
			diags.AddError(
				"Invalid OrgGatewaytemplatesValue Attribute Type",
				"While creating a OrgGatewaytemplatesValue value, an invalid attribute value was detected. "+
					"A OrgGatewaytemplatesValue must use a matching attribute type for the value. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("OrgGatewaytemplatesValue Attribute Name (%s) Expected Type: %s\n", name, attributeType.String())+
					fmt.Sprintf("OrgGatewaytemplatesValue Attribute Name (%s) Given Type: %s", name, attribute.Type(ctx)),
			)
		}
	}
	...
	for name := range attributes {
		...
		if !ok {
			diags.AddError(
				"Extra OrgGatewaytemplatesValue Attribute Value",
				"While creating a OrgGatewaytemplatesValue value, an extra attribute value was detected. "+
					"A OrgGatewaytemplatesValue must not contain values beyond the expected attribute types. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("Extra OrgGatewaytemplatesValue Attribute Name: %s", name),
			)
		}
	}

	if diags.HasError() {
		return NewOrgGatewaytemplatesValueUnknown(), diags
	}
...
func NewOrgGatewaytemplatesValueMust(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) OrgGatewaytemplatesValue {
	object, diags := NewOrgGatewaytemplatesValue(attributeTypes, attributes)
	...
	if diags.HasError() {
		...
		panic("NewOrgGatewaytemplatesValueMust received error(s): " + strings.Join(diagsStrings, "\n"))
	}
...
func (t OrgGatewaytemplatesType) ValueFromTerraform(ctx context.Context, in tftypes.Value) (attr.Value, error) {
	if in.Type() == nil {
		return NewOrgGatewaytemplatesValueNull(), nil
	}
	...
	if !in.IsKnown() {
		return NewOrgGatewaytemplatesValueUnknown(), nil
	}
	...
	return NewOrgGatewaytemplatesValueMust(OrgGatewaytemplatesValue{}.AttributeTypes(ctx), attributes), nil
}
...
var _ basetypes.ObjectValuable = OrgGatewaytemplatesValue{}
...
func (v OrgGatewaytemplatesValue) Equal(o attr.Value) bool {
	other, ok := o.(OrgGatewaytemplatesValue)
