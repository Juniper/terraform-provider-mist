		...
		Attributes: map[string]schema.Attribute{
			...
			"site_webhooks": schema.SetNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					...
					CustomType: SiteWebhooksType{
						ObjectType: types.ObjectType{
							AttrTypes: SiteWebhooksValue{}.AttributeTypes(ctx),
						},
					},
				},
				Computed: true,
...
var _ basetypes.ObjectTypable = SiteWebhooksType{}
...
func NewSiteWebhooksValue(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) (SiteWebhooksValue, diag.Diagnostics) {
	...
	for name, attributeType := range attributeTypes {
		...
		if !ok {
			diags.AddError(
				"Missing SiteWebhooksValue Attribute Value",
				"While creating a SiteWebhooksValue value, a missing attribute value was detected. "+
					"A SiteWebhooksValue must contain values for all attributes, even if null or unknown. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("SiteWebhooksValue Attribute Name (%s) Expected Type: %s", name, attributeType.String()),
			)
		...
		if !attributeType.Equal(attribute.Type(ctx)) {
			diags.AddError(
				"Invalid SiteWebhooksValue Attribute Type",
				"While creating a SiteWebhooksValue value, an invalid attribute value was detected. "+
					"A SiteWebhooksValue must use a matching attribute type for the value. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("SiteWebhooksValue Attribute Name (%s) Expected Type: %s\n", name, attributeType.String())+
					fmt.Sprintf("SiteWebhooksValue Attribute Name (%s) Given Type: %s", name, attribute.Type(ctx)),
			)
		}
	}
	...
	for name := range attributes {
		...
		if !ok {
			diags.AddError(
				"Extra SiteWebhooksValue Attribute Value",
				"While creating a SiteWebhooksValue value, an extra attribute value was detected. "+
					"A SiteWebhooksValue must not contain values beyond the expected attribute types. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("Extra SiteWebhooksValue Attribute Name: %s", name),
			)
		}
	}
...
func NewSiteWebhooksValueMust(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) SiteWebhooksValue {
	object, diags := NewSiteWebhooksValue(attributeTypes, attributes)
	...
	if diags.HasError() {
		...
		panic("NewSiteWebhooksValueMust received error(s): " + strings.Join(diagsStrings, "\n"))
	}
...
func (t SiteWebhooksType) ValueFromTerraform(ctx context.Context, in tftypes.Value) (attr.Value, error) {
	if in.Type() == nil {
		return NewSiteWebhooksValueNull(), nil
	}
	...
	if !in.IsKnown() {
		return NewSiteWebhooksValueUnknown(), nil
	}
	...
	return NewSiteWebhooksValueMust(SiteWebhooksValue{}.AttributeTypes(ctx), attributes), nil
}
...
func (v SiteWebhooksValue) ToTerraformValue(ctx context.Context) (tftypes.Value, error) {
	...
	case attr.ValueStateKnown:
		...
		val, err = v.SiteWebhooksType.ToTerraformValue(ctx)
...
func (v SiteWebhooksValue) Equal(o attr.Value) bool {
	...
	if !v.SiteWebhooksType.Equal(other.SiteWebhooksType) {
		return false
	}
