...
func (d *deviceApStatsDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Info(ctx, "Configuring Mist Org Devices AP Stats Datasource client")
...
func (d *deviceApStatsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	dataSet, diags := types.SetValue(datasource_device_ap_stats.DeviceApStatsValue{}.Type(ctx), elements)
	if diags != nil {
		diags.Append(diags...)
	}

	if err := resp.State.SetAttribute(ctx, path.Root("device_ap_stats"), dataSet); err != nil {
