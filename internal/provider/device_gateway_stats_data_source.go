...
func (d *deviceGatewayStatsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var start int
	var mType models.DeviceTypeWithAllEnum = models.DeviceTypeWithAllEnum(models.DeviceTypeWithAllEnum_GATEWAY)
	...
	for limit*page < total {
		...
		mist_stats := []models.StatsGateway{}
		json.Unmarshal(body, &mist_stats)

		diags = datasource_device_gateway_stats.SdkToTerraform(ctx, &mist_stats, &elements)
