...
func (r *orgOrgAlarmtemplateResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	alarmtemplate, diags := resource_org_alarmtemplate.TerraformToSdk(ctx, &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid \"org_id\" value for \"mist_org_alarmtemplate\" resource",
	...
	state, diags = resource_org_alarmtemplate.SdkToTerraform(ctx, data.Data)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *orgOrgAlarmtemplateResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Info(ctx, "Starting OrgAlarmtemplate Read: alarmtemplate_id "+state.Id.ValueString())
	...
	alarmtemplateId, err := uuid.Parse(state.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid \"id\" value for \"mist_org_alarmtemplate\" resource",
			fmt.Sprintf("Unable to parse the the UUID \"%s\": %s", state.Id.ValueString(), err.Error()),
		)
		return
	}
	httpr, err := r.client.OrgsAlarmTemplates().GetOrgAlarmTemplate(ctx, orgId, alarmtemplateId)
	if httpr.Response.StatusCode == 404 {
		resp.State.RemoveResource(ctx)
		return
	} else if err != nil {
		resp.Diagnostics.AddError(
			"Error getting \"mist_org_alarmtemplate\" resource",
			"Unable to get the Alarmtemplate, unexpected error: "+err.Error(),
		)
		return
	}
	state, diags = resource_org_alarmtemplate.SdkToTerraform(ctx, httpr.Data)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *orgOrgAlarmtemplateResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Info(ctx, "Starting OrgAlarmtemplate Update for OrgAlarmtemplate "+plan.Id.ValueString())
	orgId, err := uuid.Parse(state.OrgId.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid \"org_id\" value for \"mist_org_alarmtemplate\" resource",
...
func (r *orgOrgAlarmtemplateResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Info(ctx, "Starting OrgAlarmtemplate Delete: alarmtemplate_id "+state.Id.ValueString())
	orgId, err := uuid.Parse(state.OrgId.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid \"org_id\" value for \"mist_org_alarmtemplate\" resource",
...
func (r *orgOrgAlarmtemplateResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if len(importIds) != 2 {
		resp.Diagnostics.AddError(
			"Invalid \"id\" value for \"mist_org_alarmtemplate\" resource",
			"import \"id\" format must be \"{org_id}.{alarmtemplate_id}\"",
		)
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid \"org_id\" value for \"mist_org_alarmtemplate\" resource",
			fmt.Sprintf("Unable to parse the the UUID \"%s\": %s. Import \"id\" format must be \"{org_id}.{alarmtemplate_id}\"", importIds[0], err.Error()),
		)
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid \"id\" value for \"mist_org_alarmtemplate\" resource",
			fmt.Sprintf("Unable to parse the the UUID \"%s\": %s. Import \"id\" format must be \"{org_id}.{alarmtemplate_id}\"", importIds[1], err.Error()),
		)
		return
	}
