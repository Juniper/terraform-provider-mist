...
import (
	...
	"github.com/Juniper/terraform-provider-mist/internal/datasource_org_networks"
...
func (d *orgNetworksDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Info(ctx, "Configuring Mist Org Networks Datasource client")
...
func (d *orgNetworksDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_org_networks"
}

func (d *orgNetworksDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *orgNetworksDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid \"org_id\" value for \"org_networks\" data_source",
	...
	for limit*page < total {
		...
		diags = datasource_org_networks.SdkToTerraform(ctx, &data.Data, &elements)
	...
	dataSet, diags := types.SetValue(datasource_org_networks.OrgNetworksValue{}.Type(ctx), elements)
	if diags != nil {
		diags.Append(diags...)
	}

	if err := resp.State.SetAttribute(ctx, path.Root("org_networks"), dataSet); err != nil {
