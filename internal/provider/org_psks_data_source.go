...
func (d *orgPsksDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: docCategoryWlan + "This data source provides the list of WAN Assurance Psks.\n" +
...
func (d *orgPsksDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for limit*page < total {
		...
		if err != nil {
			resp.Diagnostics.AddError(
				...
				"Unable to get the the list of Org PSKs, unexpected error: "+err.Error(),
			)
			return
		}
		...
		diags = datasource_org_psks.SdkToTerraform(ctx, &data.Data, &elements)
	...
	dataSet, diags := types.SetValue(datasource_org_psks.OrgPsksValue{}.Type(ctx), elements)
	if diags != nil {
		diags.Append(diags...)
	}

	if err := resp.State.SetAttribute(ctx, path.Root("org_psks"), dataSet); err != nil {
