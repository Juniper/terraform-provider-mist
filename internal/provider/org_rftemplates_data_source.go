...
import (
	...
	"terraform-provider-mist/internal/datasource_org_rftemplates"
...
func (d *orgRftemplatesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	deviceApStat, diags := datasource_org_rftemplates.SdkToTerraform(ctx, data.Data)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	if err := resp.State.SetAttribute(ctx, path.Root("org_rftemplates"), deviceApStat); err != nil {
