...
func (d *orgServicesDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_org_services"
}

func (d *orgServicesDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	...
	for limit*page < total {
		...
		if err != nil {
			resp.Diagnostics.AddError(
				...
				"Unable to get the the list of Services, unexpected error: "+err.Error(),
			)
			return
		}
