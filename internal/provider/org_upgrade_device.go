...
func (r *upgradeDeviceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	state, diags = r.refreshFwUpdate(ctx, state)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *upgradeDeviceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if state.Status.ValueString() == "inprogress" {
		diags.AddError(
			"Unable to start a new device upgrade",
			fmt.Sprintf("Device upgrade for device %s is alreay in progress, cannot update it...\n"+
				"Please wait until the end of the current upgrade process and try again."))
		return
	} else {
		siteId, err := uuid.Parse(plan.SiteId.ValueString())
		if err != nil {
...
func (r *upgradeDeviceResource) refreshFwUpdate(ctx context.Context, state resource_upgrade_device.UpgradeDeviceModel) (resource_upgrade_device.UpgradeDeviceModel, diag.Diagnostics) {
	...
	state, diags = resource_upgrade_device.DeviceStatSdkToTerraform(ctx, state, &data.Data)
	diags.Append(diags...)
	return state, diags
}
