...
func (d *sitePsksDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_site_psks"
}

func (d *sitePsksDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	...
	for limit*page < total {
		...
		if err != nil {
			resp.Diagnostics.AddError(
				...
				"Unable to get the the list of Site PSKs, unexpected error: "+err.Error(),
			)
			return
		}
