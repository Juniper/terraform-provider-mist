...
func (d *siteWebhooksDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_site_webhooks"
}

func (d *siteWebhooksDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	...
	for limit*page < total {
		...
		if err != nil {
			resp.Diagnostics.AddError(
				...
				"Unable to get the the list of Site Webhooks, unexpected error: "+err.Error(),
			)
			return
		}
