...
func wanProbeOverridePortVpnPathTerraformToSdk(ctx context.Context, diags *diag.Diagnostics, data basetypes.ObjectValue) *models.GatewayWanProbeOverride {
	...
	if !plan.Hostnames.IsNull() && !plan.Hostnames.IsUnknown() {
		result.Hostnames = mistutils.ListOfStringTerraformToSdk(plan.Hostnames)
	}
...
func portConfigTerraformToSdk(ctx context.Context, diags *diag.Diagnostics, data basetypes.MapValue) map[string]models.GatewayPortConfig {
	...
	for key, val := range data.Elements() {
		...
		if portConfig.PoeKeepStateWhenReboot.ValueBoolPointer() != nil {
			result.PoeKeepStateWhenReboot = portConfig.PoeKeepStateWhenReboot.ValueBoolPointer()
		}
		...
		if portConfig.WanSpeedtestMode.ValueStringPointer() != nil {
			result.WanSpeedtestMode = models.ToPointer(models.GatewayPortConfigWanSpeedtestModeEnum(portConfig.WanSpeedtestMode.ValueString()))
		}
