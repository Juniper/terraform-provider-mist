...
func TerraformToSdk(ctx context.Context, plan *OrgDeviceprofileSwitchModel) (models.Deviceprofile, diag.Diagnostics) {
	data := models.DeviceprofileSwitch{}
	...
	if plan.EvpnConfig.IsNull() || plan.EvpnConfig.IsUnknown() {
		...
		data.EvpnConfig = evpnConfigTerraformToSdk(plan.EvpnConfig)
	}
	...
	if plan.IotConfig.IsNull() || plan.IotConfig.IsUnknown() {
		unset["-iot_config"] = ""
	} else {
		data.IotConfig = iotConfigTerraformToSdk(plan.IotConfig)
	}
	...
	deviceprofile := models.DeviceprofileContainer.FromDeviceprofileSwitch(data)
