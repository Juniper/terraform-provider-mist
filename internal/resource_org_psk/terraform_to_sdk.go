...
func TerraformToSdk(ctx context.Context, plan *OrgPskModel) (models.Psk, diag.Diagnostics) {
	...
	if !plan.MaxUsage.IsNull() && !plan.MaxUsage.IsUnknown() {
		data.MaxUsage = models.ToPointer(int(plan.MaxUsage.ValueInt64()))
	} else {
		unset["-max_usage"] = ""
	}
